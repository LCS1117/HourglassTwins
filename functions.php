...
function SimPaled_send_email($parent_id, $comment)
{
    ...
    if ($spam_confirmed != 'spam' && $to != $admin_email && $to != $author_email) {
        ...
        $subject = '您於' . get_the_title($comment->comment_post_ID) . '」文章的留言，有了新的回應';
        $message = '<p>您好，<code>' . trim(get_comment($parent_id)->comment_author) . '</code></p>' .
            '<p>您曾在 <code>' . get_option('blogname') . '</code> 的「' . get_the_title($comment->comment_post_ID) . '」中留下過留言:</p>' .
            '<p class="com-content">' . trim(get_comment($parent_id)->comment_content) . '</p>' .
            '<br><p>' . trim($comment->comment_author) . ' 給您的回復如下：</p>' .
            '<p class="com-content">' . trim($comment->comment_content) . '</p>' .
            '<p style="border-top: 1px solid #DDDDDD; padding-top:6px; margin-top:15px; color:#838383;">您可以點擊此連結<a href="' . htmlspecialchars(get_comment_link($parent_id, array("type" => "all"))) . '">查看完整内容</a>| 歡迎再次來訪<a href="' . get_option('home') . '">' . get_option('blogname') . '</a></p>' .
            '<p style="color: #bbb;margin-top: 40px;">請不要回覆該郵件，它是由程式自動發出的。</p>' .
        ...
        <div class="media-body">
            ...
            <div class="comment-metadata">
   			<span class="comment-pub-time">
                ...
                if ($comment->comment_approved == '0') echo "<em>你的留言需等待審核…</em>" ?>
   			</span>
                <?php comment_reply_link(array_merge($args, array('reply_text' => '<i class=\"fas fa-reply\"></i> 回復', 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?> <?php edit_comment_link(__('(Edit)'), '&nbsp;&nbsp;', ''); ?>
            </div>
